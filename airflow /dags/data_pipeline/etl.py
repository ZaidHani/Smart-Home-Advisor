...
###
    ...
    print(f"data length is :{len(products_df)}")
    for i in range(len(products_df)):
        
        location = geolocator.reverse((products_df['lat'][i], products_df['long'][i]), exactly_one=True)
    ...
    products_df['country'] = country_list
    products_df['city_2'] = city_list

    cities = ['عمان', 'الزرقاء', 'البلقاء', 'إربد', 'عجلون', 'جرش', 'العقبة', 'الطفيلة', 'معان', 'المفرق', 'الكرك', 'مادبا']

    products_df = products_df[products_df['country'] == 'الأردن']
    products_df = products_df[products_df['city_2'] != '']
    products_df = products_df[products_df['city_2'].isin(cities)]

    products_df['city'] = products_df['city_2']
    products_df.drop(['city_2', 'country'], axis = 1, inplace = True)
    products_df['city'].replace(['عمان', 'الزرقاء', 'البلقاء', 'إربد', 'عجلون', 'جرش', 'العقبة', 'الطفيلة', 'معان', 'المفرق', 'الكرك', 'مادبا'],
                                ['Amman', 'Zarqa', 'Balqa', 'Irbid', 'Ajloun', 'Jerash', 'Aqaba', 'Tafila', "Ma'an", 'Mafraq', "Al Karak", 'Madaba'], 
                                inplace = True)
