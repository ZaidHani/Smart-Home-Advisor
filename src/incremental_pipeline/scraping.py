...
def scrape_links(URL:str, pages:int) -> str:
    ...
    pd.DataFrame(df).to_csv('D:/gproject/data/incremental/links.csv',index_label=False,mode='w')
...
def scrape_prodcuts_data(links:pd.DataFrame) -> str:
    ...
    for row in range(len(links)):
        ...
        df.append(data)
        if i==len(links):
            pd.DataFrame(df).to_csv(f'D:/gproject/data/incremental/products.csv',index_label=False, mode='w')
        i+=1
        if i%100==0:
...
def main():
    ...
    links = pd.read_csv('D:/gproject/data/incremental/links.csv')
